...
const DESIGN_WIDTH = 780; // reference width the base circle sizes were tuned against
...
export default function SizeDiscriminationScreen() {
  ...
  useEffect(() => {
    return () => {
      ...
      if (nextTrialRef.current) clearTimeout(nextTrialRef.current);
    };
  }, []);

  const playAreaWidth = Math.min(width - Spacing.lg * 2, 460);
  ...
  const beginTrial = useCallback(
    (index: number) => {
      ...
      if (biggerOnRight) {
        ...
        setRightSize(Math.round(baseSize * (1 + diffPercent)));
        setCorrectSide('right');
      } else {
        setLeftSize(Math.round(baseSize * (1 + diffPercent)));
      ...
      timeoutRef.current = setTimeout(() => {
        if (roundId !== roundIdRef.current || answeredRef.current) return;
        handleTimeout(index, diffPercent);
      }, TIMER_DURATION);

      // store the diff for this trial so respond()/timeout() can reference it
  ...
  const respond = useCallback(
    (side: Side) => {
      ...
      setResponses((prev) => [...prev, { correct, rt, diff }]);
      setLastRt(rt);

      if (correct) {
        setScore((prev) => prev + 10);
        setFeedback({
          text: language === 'fa' ? 'درست بود ✓' : 'Correct ✓',
          type: 'correct',
        });
      } else {
        setFeedback({
          text:
            language === 'fa'
              ? `اشتباه بود — ${correctSide === 'left' ? (isRTL ? 'راست' : 'left') : isRTL ? 'راست' : 'left'}`
              : 'Wrong answer',
          type: 'wrong',
        });
      }
      ...
      setDetailText(
        language === 'fa'
          ? `تفاوت اندازه: ${Math.round(diff * 100)}٪  ·  زمان پاسخ: ${rt}ms`
          : `Size difference: ${Math.round(diff * 100)}%  ·  RT: ${rt}ms`
      );
    ...
    [correctSide, goToNextTrial, isReady, isRTL, language, trialIndex]
  );
  ...
  const handleTimeout = useCallback(
    (index: number, diff: number) => {
      ...
      setResponses((prev) => [...prev, { correct: false, rt: 0, diff }]);
      setFeedback({
        text: language === 'fa' ? 'زمان تمام شد' : 'Time is up',
        type: 'timeout',
      });
      setDetailText(
        language === 'fa'
          ? `تفاوت اندازه: ${Math.round(diff * 100)}٪`
          : `Size difference: ${Math.round(diff * 100)}%`
      );
  ...
  const startGame = () => {
    ...
    setScore(0);
    setResponses([]);
    nextTrialRef.current = setTimeout(() => beginTrial(0), 400);
  };
  ...
  const corrects = responses.filter((r) => r.correct).length;
  const accuracy = total > 0 ? (corrects / total) * 100 : 0;
  const validRts = responses.filter((r) => r.correct && r.rt > 0).map((r) => r.rt);
  const avgRt = validRts.length > 0 ? validRts.reduce((a, b) => a + b, 0) / validRts.length : 0;

  const getThresholdLabel = () => {
    const buckets: Record<string, { correct: number; total: number }> = {};
    responses.forEach((r) => {
      const bucketStart = Math.floor((r.diff * 100) / 5) * 5;
      const key = `${bucketStart}`;
      if (!buckets[key]) buckets[key] = { correct: 0, total: 0 };
      buckets[key].total += 1;
      if (r.correct) buckets[key].correct += 1;
    });
    ...
    return language === 'fa' ? 'داده کافی نیست' : 'Not enough data';
  };
  ...
  const timerColor = timerAnim.interpolate({
    inputRange: [0, 0.3, 0.301, 1],
    outputRange: [colors.error, colors.error, colors.primary, colors.primary],
  });

  const leftDiameter = leftSize * 2 * scale;
  const rightDiameter = rightSize * 2 * scale;
      ...
      >
        ...
        <View style={styles.levelHeader}>
          ...
          <Text allowFontScaling={false} style={[styles.title, { color: colors.text, textAlign: 'center' }]}>
            {language === 'fa' ? 'تشخیص اندازه' : 'Size Discrimination'}
          </Text>
          <Text
            allowFontScaling={false}
            style={[styles.subtitle, { color: colors.textSecondary, textAlign: 'center' }]}
          >
            {language === 'fa'
        ...
        <View style={[styles.instructionCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          >
            {language === 'fa' ? 'راهنما' : 'How to play'}
          </Text>
        ...
        <View style={[styles.statsPreview, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
          <View style={[styles.statPreviewItem, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Target size={18} color={colors.primary} />
            <Text allowFontScaling={false} style={[styles.statPreviewText, { color: colors.text }]}>
              {TOTAL_TRIALS} {language === 'fa' ? 'مرحله' : 'trials'}
            </Text>
          </View>
          <View style={[styles.statPreviewItem, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Clock size={18} color={colors.primary} />
            <Text allowFontScaling={false} style={[styles.statPreviewText, { color: colors.text }]}>
              {language === 'fa' ? '۸ ثانیه' : '8 seconds'}
            </Text>
          </View>
          <View style={[styles.statPreviewItem, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Trophy size={18} color={colors.primary} />
            <Text allowFontScaling={false} style={[styles.statPreviewText, { color: colors.text }]}>
              +10 {language === 'fa' ? 'به ازای هر پاسخ درست' : 'per correct'}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          <Text allowFontScaling={false} style={styles.startButtonText}>
            {language === 'fa' ? 'شروع تست' : 'Start Test'}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    );
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.resultOverlay}>
          <View style={[styles.resultCard, { backgroundColor: colors.surface }]}>
            ...
            <Text allowFontScaling={false} style={[styles.resultTitle, { color: colors.text, textAlign: 'center' }]}>
              {language === 'fa' ? 'آزمون کامل شد!' : 'Test complete!'}
            </Text>

            <View style={styles.resultStatsGrid}>
              <View style={[styles.resultStatBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
                <Text allowFontScaling={false} style={[styles.resultStatValue, { color: colors.primary }]}>
                  {corrects}/{total}
                </Text>
                <Text allowFontScaling={false} style={[styles.resultStatLabel, { color: colors.textSecondary }]}>
                  {language === 'fa' ? 'پاسخ درست' : 'Correct'}
                </Text>
              </View>
              <View style={[styles.resultStatBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
                <Text allowFontScaling={false} style={[styles.resultStatValue, { color: colors.primary }]}>
              ...
              <View style={[styles.resultStatBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
                ...
                <Text allowFontScaling={false} style={[styles.resultStatLabel, { color: colors.textSecondary }]}>
                  {language === 'fa' ? 'میانگین زمان (ms)' : 'Avg RT (ms)'}
                </Text>
              </View>
              <View style={[styles.resultStatBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
                <Text allowFontScaling={false} style={[styles.resultStatValue, { color: colors.primary }]}>
                ...
                <Text allowFontScaling={false} style={[styles.resultStatLabel, { color: colors.textSecondary }]}>
                  {language === 'fa' ? 'آستانه تشخیص' : 'Threshold'}
                </Text>
              </View>
            </View>

            <Text allowFontScaling={false} style={[styles.finalScore, { color: colors.primary, textAlign: 'center' }]}>
              {score} {language === 'fa' ? 'امتیاز' : 'Points'}
            </Text>

            <TouchableOpacity onPress={restartGame} style={[styles.resultButton, { backgroundColor: colors.primary }]}>
              <RotateCcw size={16} color="#fff" />
              <Text allowFontScaling={false} style={styles.resultButtonText}>
                {language === 'fa' ? 'دوباره تلاش کن' : 'Try Again'}
              </Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={backToIntro} style={[styles.secondaryButton, { borderColor: colors.border }]}>
              <Text
                allowFontScaling={false}
                style={[styles.secondaryButtonText, { color: colors.text, textAlign: 'center' }]}
              >
                {language === 'fa' ? 'بازگشت به راهنما' : 'Back to Instructions'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
  ...
  return (
    <View style={[styles.gameContainer, { backgroundColor: colors.background }]}>
      ...
      <Text allowFontScaling={false} style={[styles.infoText, { color: colors.text }]}>
        {language === 'fa' ? 'کدام دایره بزرگ‌تر است؟' : 'Which circle is bigger?'}
      </Text>
      ...
      >
        <View style={[styles.circleColumn, { marginRight: spacing / 2 }]}>
          <Animated.View style={{ transform: [{ scale: circlePulse }] }}>
            <TouchableOpacity
              activeOpacity={0.85}
              disabled={!isReady}
              hitSlop={{ top: 14, bottom: 14, left: 14, right: 14 }}
              onPress={() => respond('left')}
            >
              <LinearGradient
                colors={['#FF8A8A', '#E53935', '#B71C1C']}
                style={[
                  styles.circle,
                  { width: leftDiameter, height: leftDiameter, borderRadius: leftDiameter / 2 },
                ]}
              />
            </TouchableOpacity>
          </Animated.View>
          <Text allowFontScaling={false} style={[styles.circleLabel, { color: colors.textSecondary }]}>
            {language === 'fa' ? 'چپ' : 'Left'}
          </Text>
        </View>

        <View style={[styles.circleColumn, { marginLeft: spacing / 2 }]}>
          <Animated.View style={{ transform: [{ scale: circlePulse }] }}>
            <TouchableOpacity
              activeOpacity={0.85}
              disabled={!isReady}
              hitSlop={{ top: 14, bottom: 14, left: 14, right: 14 }}
              onPress={() => respond('right')}
            >
              <LinearGradient
                colors={['#7FD8FF', '#1E88E5', '#0D47A1']}
                style={[
                  styles.circle,
                  { width: rightDiameter, height: rightDiameter, borderRadius: rightDiameter / 2 },
                ]}
              />
            </TouchableOpacity>
          </Animated.View>
